...
public class LstSubdeadlineAlgorithm extends DatacenterBroker{
	...
	public LstSubdeadlineAlgorithm(String name) throws Exception {		
		super(name);
...
//			throw new RuntimeException();
			...
			for(MyTask t: p) {
				if(idleVm.submitTaskflag == true) {
					...
					if(t.getStatus() == MyTask.STATUS_READY) {
						...
						printLog("TASK(P) SUBMMITION ON BROKER:/ " + " Ready task# " + t.getCloudletId() + " with Lst " + t.getLstTimeTask()+ " Scheduled on vm# " + idleVm.getId()+ " with subdeadline " + t.getsubDeadline());		
	...
	private boolean submitTaskVm(MyTask readyTask, MyVm vm) {		
		if(vm.submitTaskflag == true) {
			...
			printLog("TASK SUBMMITION ON BROKER:/ " + " Ready task# " + readyTask.getCloudletId() + " with Lst "+ readyTask.getLstTimeTask() + " Scheduled on vm# " + vm.getId() + " with subdeadline " + readyTask.getsubDeadline());
	...
	private void initialBroker(SimEvent ev) throws Exception {
		...
		/*set DaxPath*/
		String daxpath = "F:/master/term 4/implementation/cloudsim-3.0/examples/dax/old/Montage_25.xml";
...
//		w.calculateTimeTasks(fastVmMips);
//		
//		w.calculateResourceLevel(fastVmMips);
		
		int totalVMprovisin = 0;
		for(Entry<Integer, ArrayList<MyTask>> entry: w.NumTaskafterSchedule.entrySet()) {
			int level = entry.getKey();
			if(w.estLevel.get(level) < MyConstant.TIME_CYCLE) {
				int currentlevel = w.numVminLevel.get(level);
				totalVMprovisin += currentlevel;
			}
		}
		for(int i = 0; i < totalVMprovisin; i++) {
